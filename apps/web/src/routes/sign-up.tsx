...
function SignUpPage() {
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Start registration process (only request Email and Password, no phone number!)
      await signUp.create({
        emailAddress: email,
        password,
        firstName,
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (completeSignUp.status === 'complete') {
        ...
        console.log('SignUp complete status:', completeSignUp.status)
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: 'var(--on-surface-variant)', fontSize: 14, margin: 0 }}>
            {pendingVerification ? `Enter the OTP sent to ${email}` : 'Sign up to build your case dossier'}
          </p>
        </div>
        ...
        >
          ...
          {!pendingVerification ? (
            /* SIGN UP FORM */
            <form onSubmit={handleSignUpSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div>
